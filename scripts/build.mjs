...
function makeProvider(providerId, displayName) {
  ...
  for (const row of master.rows) {
    ...
    const services = (row.providers?.[providerId] || []).map((name) => ({
      id: (providerId + "_" + name).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, ""),
      name,
      category: cat
    }));
...
fs.writeFileSync(path.join(outDir, "catalog.generated.js"), js, "utf8");
fs.writeFileSync(path.join(outDir, "version.generated.js"), `window.ATK_VERSION=${JSON.stringify({version: master.catalog_version, generatedAt: catalog.generatedAt})};\n`, "utf8");

console.log("Built web/generated/catalog.generated.js");
